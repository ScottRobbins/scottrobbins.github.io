...
Looking at the query above, you can see we gave it a name of `getBooksWithAuthors`. It's requesting all of the books, and for each book it's reuesting its `id`, `title`, and `author`. From the author, it's asking for the `id`, `name`, and `age`. Notice that the `Author` type would also let a query reuest all of the books by that author, but this query is _not_ requesting it. This is something fundamental to GraphQL; It lets you request all of the data you want from the entire schema, and **only** what you want. 
...
This would give the guest information under a `included` key in the response. 
